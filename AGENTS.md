...
- **Figma `2036:31121` (“Feedback”):** MCP returns a **stack** of rows (last-modified strip, rating bar, page-feedback bar). Only the **page feedback** row is implemented as `PageFeedbackBar`; the other rows are separate future slices unless the issue explicitly includes them.
