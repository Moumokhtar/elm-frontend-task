...
- [ ] `ng new` project runs with standalone components and routing enabled
- [ ] Bootstrap 5 CSS imported in `angular.json` styles array
- [ ] PrimeNG installed, `provideAnimations()` and `providePrimeNG()` in `app.config.ts`
- [ ] `html` tag has `dir="rtl"` and `lang="ar"`
- [ ] Global stylesheet sets `body { direction: rtl; font-family: ... }`
- [ ] Routes defined for `/` (home), `/service` (detail), `/form` (form page)
